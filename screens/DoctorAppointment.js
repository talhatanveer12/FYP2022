    ...
    return (
      <View key={index} style={styles.container1}>
        <View style={styles.cardContainer}>
          ...
          >
            <View style={{ flex: 1 }}>
              <Text style={styles.titleStyle}>{item.DoctorName}</Text>
              <Text style={styles.categoryStyle}>
                {item.AppointmentDisease}
              </Text>
            </View>
            <Image
          ...
          <View style={{ flex: 4, top: 10 }}>
            ...
            >
              ...
              >
                ...
                />
                <Text style={{ left: 3 }}>{item.AppointmentDate}</Text>
              </View>
              <View
                style={{
                  flex: 1,
              ...
              >
                ...
                />
                <Text style={{ left: 3 }}>{item.AppointmentTime}</Text>
              </View>
              <View
                style={{
                  flex: 1,
            ...
            >
              <TouchableOpacity
                ...
                style={item.status != "reject" ? styles.button : styles.button1}
                onPress={Accept}
              >
      ...
      .then((snapshot) => {
        if (snapshot.exists()) {
          snapshot.forEach((child) => {
            ...
            if (child.val().DoctorEmail == `${global.DoctorEmail}`) {
              console.log("sdsdsdsdsdsdsdsdsdsd");
              console.log(child.val().UserCNIC);
              main.push({
                DoctorName: child.val().DoctorName,
                AppointmentDisease: child.val().AppointmentDisease,
                AppointmentDate: child.val().AppointmentDate,
                AppointmentTime: child.val().AppointmentTime,
                UserCINC: child.val().UserCNIC,
